...
def train_model(model, optimizer, train_loader, model_func, lr_scheduler, optim_cfg,
    ...
    Args:
        ...
        merge_all_iters_to_one_epoch: 是否将所有iter合并为一个epoch 就是所有epochs合并成为一个epoch
    """
    accumulated_iter = start_iter # 累计已迭代次数
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        total_it_each_epoch = len(train_loader) # 完成一个epoch所需的iter次数：928  也就是batch的大小   总的iter数等于一次epoch的iter*epoch
