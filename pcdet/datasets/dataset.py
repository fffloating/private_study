    ...
    def prepare_data(self, data_dict):
        ...
        Returns:
            data_dict:
                ...
                ...
        """
        # 训练模式下，对存在于class_name中的数据进行增强 训练模式和其他模式分开。
        if self.training:
            ...
            data_dict = self.data_augmentor.forward(   #  11111111111111  进行数据增强
                data_dict={
                    **data_dict,
        ...
        if data_dict.get('points', None) is not None:
            data_dict = self.point_feature_encoder.forward(data_dict)  # 222222222222   对数据进行PFE操作

        # 对点云进行预处理，包括移除超出point_cloud_range的点、 打乱点的顺序以及将点云转换为voxel
        data_dict = self.data_processor.forward(   # 3333333333 进行预处理操作
            data_dict=data_dict
        )
        ...
        for key, val in data_dict.items():
            try:
                ...
                if key in ['voxels', 'voxel_num_points']:
                    ...
                    batch_gt_boxes3d = np.zeros((batch_size, max_gt, val[0].shape[-1]),
                                                dtype=np.float32)  # 构造空的box3d矩阵（B, N, 7）
