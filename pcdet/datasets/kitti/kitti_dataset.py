...
class KittiDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        Args:
            ...
            logger:
        """
        # 初始化类，将参数赋值给类的属性  因为这时候有继承  所以这时候一定会先初始化父类的  先调用dataset里面的基类
        super().__init__(
    ...
    def __len__(self):
        # 如果合并到一个epoch中  那么 数据集的长度等于每个长度乘以总的epochs数
    ...
    def __getitem__(self, index):  # 这个什么时候被调用的？？？？？？？？？
        """
        ...
        # 取出第index帧的信息
        info = copy.deepcopy(self.kitti_infos[index])  # info: image
        ...
        if 'annos' in info:
            ...
            # boxes3d_camera: (N, 7) [x, y, z, l, h, w, r] in rect camera coords  得到一个
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_kitti_infos':
        ...
        create_kitti_infos(
            ...
            class_names=['Car', 'Pedestrian', 'Cyclist'],  # 是个列表
            data_path=ROOT_DIR / 'data' / 'kitti',  # OpenPCDet/data/kitti  数据的路径
            save_path=ROOT_DIR / 'data' / 'kitti'  # OpenPCDet/data/kitti  输出数据的路径
        )
