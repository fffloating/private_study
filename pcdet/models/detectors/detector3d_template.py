...
class Detector3DTemplate(nn.Module):
    def __init__(self, model_cfg, num_class, dataset):  # 初始化函数
        super().__init__()  # 继承函数
    ...
    def build_vfe(self, model_info_dict):
        ...
        if self.model_cfg.get('VFE', None) is None:
            return None, model_info_dict    # 输出为元组
        ...
        # 将模块添加道module_list中
        model_info_dict['module_list'].append(vfe_module)  # 列表中增加上vfe_module这个  为什么不用extend？
