...
class AnchorHeadTemplate(nn.Module):
    ...
    @staticmethod  # 静态方法  即使不实例化对象 也可以进行函数的调用
    ...
    def get_target_assigner(self, anchor_target_cfg):  # 得到目标分配器
        # target_assigner初始化
    ...
    def assign_targets(self, gt_boxes):  # 进行分配目标  对真值框和anchor进行匹配分配  根据IOU区分正负样本
        """
        Args:
            gt_boxes: (B, M, 8)
        Returns:
    ...
    def get_direction_target(anchors, reg_targets, one_hot=True, dir_offset=0, num_bins=2):
        ...
        if one_hot:
            ...
            dir_targets.scatter_(-1, dir_cls_targets.unsqueeze(dim=-1).long(),
                                 1.0)  # onehot编码，只存在两个方向:正向和反向 (4, 321408, 2)
    ...
    def generate_predicted_boxes(self, batch_size, cls_preds, box_preds, dir_cls_preds=None):  # 根据预测结果生成预测框
        """
        Args:
            batch_size:
            cls_preds: (N, H, W, C1)
        ...
        batch_cls_preds = cls_preds.view(batch_size, num_anchors, -1).float() \
            if not isinstance(cls_preds,
                              list) else cls_preds  # cls_preds:（1, 248, 216, 18）--> batch_cls_preds:(1, 321408, 3)
        ...
        if dir_cls_preds is not None:
            ...
            batch_box_preds[..., 6] = dir_rot + dir_offset + period * dir_labels.to(
                batch_box_preds.dtype)  # 转化0.25pi到2.5pi
